        ...
        <section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {candidates
              ...
              >
                {candidate.photoUrl && (
                  <img src={`https://onlinevoting-tau.vercel.app/${candidate.photoUrl.replace('\\', '/')}`} alt={candidate.name} style={{ width: '50px', height: '50px', borderRadius: '50%', objectFit: 'cover' }} />
                )}
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', margin: '0 0 0.2rem 0' }}>
                    {candidate.partySymbolUrl && (
                      <img src={`https://onlinevoting-tau.vercel.app/${candidate.partySymbolUrl.replace('\\', '/')}`} alt={candidate.partyName} style={{ height: '18px', width: '18px', objectFit: 'contain' }} />
                    )}
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }} onClick={() => setSelectedCandidate(null)}>
            <motion.div initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="glass-card" style={{ width: '600px', background: 'white' }} onClick={(e) => e.stopPropagation()}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
                <img src={`https://onlinevoting-tau.vercel.app/${selectedCandidate.photoUrl.replace('\\', '/')}`} alt={selectedCandidate.name} style={{ width: '80px', height: '80px', borderRadius: '50%', objectFit: 'cover' }} />
                <div>
              ...
              <div style={{ background: '#F8FAFC', padding: '1.5rem', borderRadius: '12px', marginBottom: '2rem' }}>
                ...
                <a href={`https://onlinevoting-tau.vercel.app/${selectedCandidate.affidavit.affidavitFileUrl}`} target="_blank" rel="noreferrer" style={{ color: '#0076CE', textDecoration: 'none', fontWeight: 'bold' }}>📄 View PDF Affidavit</a>
              </div>
